...
#define HexToRGB(hex) color::ConvertHexToRGBA(hex, 255)
#define HexToRGBA(hex, alpha) color::ConvertHexToRGBA(hex, alpha)
...
namespace color
{
	...
	inline SDL_Color ConvertHexToRGBA(const std::string& hex, uint8_t alpha)
	{
		std::string s = hex;
